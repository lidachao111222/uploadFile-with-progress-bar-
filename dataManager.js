...
module.exports = {
    ...
    //feedbackSave
    saveFeedback(ip, data, callback) {
        var ip = ip;
        var data = data;


        var sqlStr = 'insert into feedback (ip,msg) values (?, ?)';
        connection.query(sqlStr, [ip, data], function (err, results) {
            if (err) return callback(true, ip, data);
            callback(false, ip, data);
        });
    },
    ...
    getFeedback(callback) {
        ...
        const sqlStr = 'select * from feedback';
